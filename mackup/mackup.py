...
from mackup import appsdb, config, utils
...
class Mackup:
    ...
    def check_for_usable_environment(self) -> None:
        ...
        if not Path(self._config.path).is_dir():
    ...
    def check_for_usable_restore_env(self) -> None:
        ...
        if not Path(self.mackup_folder).is_dir():
            utils.error(
                (
    ...
    def create_mackup_home(self) -> None:
        ...
        if not Path(self.mackup_folder).is_dir():
            if utils.confirm(
                ...
                Path(self.mackup_folder).mkdir(parents=True, exist_ok=True)
            else:
                utils.error("Mackup can't do anything without a home =(")

    def get_apps_to_backup(self) -> set:
        """
